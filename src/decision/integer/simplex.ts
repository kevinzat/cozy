 ...
 */
export function SimplexMethod(A: Tableau, c: number[], cols: number[]): boolean {
  if (A.col0 === undefined)
 ...
 */
export function FirstProfitable(A: Tableau, c: number[], cols: number[]): number {
...
function Zero(A: Tableau, row1: number, row2: number, col: number): void {
  ...
  if (t === 0) {  // A[row1][col] is the gcd
    if (A.entries[row2][col] % A.entries[row1][col] !== 0)
